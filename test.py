...
def test_model(
    ...
    log_and_print("emotions: " + str(emotions))
    log_and_print("Confusion Matrix:")
    log_and_print(str(conf_matrix))
    log_and_print(f"Accuracy: {accuracy:.4f}")
    log_and_print(f"Precision: {precision:.4f}")
    log_and_print(f"Recall: {recall:.4f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save_root",
        type=str,
        default="/mnt/public/gxj_2/EmoNet_2B/saved_test",
    ...
    parser.add_argument(
        "--load_key",
        type=str,
        default="/mnt/public/gxj_2/EmoNet_2B/saved/seed1_MER24_AV/AV_Base_v2/2025-04-15-13-32-55",
    ...
    if args.model not in config.MODEL_DIR_DICT.keys():
        config.MODEL_DIR_DICT[args.model] = {}
    ...
    log_and_print("====== load info and config =======")
    ...
    log_and_print("load model: {} \n".format(args.model))
    log_and_print("load model weight: {} \n".format(model_path))

    log_and_print("====== Evaluation =======")
