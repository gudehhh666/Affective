...
from toolkit.utils.loss import CELoss, CELossWithLabelSmoothing, MSELoss
...
def train_or_eval_model(
    ...
    if train:
        record_lr = optimizer.state_dict()["param_groups"][0]["lr"]
    ...
    for iter, data in enumerate(dataloader):
        ...
        # read data + cuda
        audios, videos, emos, vals, bnames = data
        ...
        if args.train_input_mode == "input_gt":
            features, emos_out, vals_out, interloss = model([batch, emos])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="save suffix as time, default: test",
    )
    parser.add_argument(
        "--save_model",
        action="store_true",
    ...
    parser.add_argument(
        "--lr_end", type=float, default=1e-5, metavar="lr_end", help="set lr_end rate"
    )
    ...
    parser.add_argument(
        "--num_workers", type=int, default=8, metavar="nw", help="number of workers"
    )
    parser.add_argument(
    ...
    if args.save_as_time:
        ...
        save_dir = os.path.join(args.save_root, args.dataset, args.model, "test")
    ...
    model_config = OmegaConf.load("models/model-tune.yaml")["uni_settings"]
    model_config = OmegaConf.to_container(model_config, resolve=True)
    model_config["video_dim"] = int(dataset_info["feat_dim"]["video_dim"])
    model_config["audio_dim"] = int(dataset_info["feat_dim"]["audio_dim"])
    for feat_name in ["video", "audio"]:
        logging.info(
    ...
    train_loader = get_dataloader_train(
        ...
        emo_rule=emo_rule,
        audio_feat_paths=train_info["audio_feat"],
        video_feat_paths=train_info["video_feat"],
    ...
    valid_loader = get_dataloader_valid(
        ...
        emo_rule=emo_rule,
        audio_feat_paths=valid_info["audio_feat"],
        video_feat_paths=valid_info["video_feat"],
        batch_size=args.batch_size,
        num_workers=args.num_workers,
        shuffle=False,
    )
    ...
    # if args.model in ["AV_Base_v3"]:
    #     cls_loss = CELossWithLabelSmoothing().cuda()
    # else:
    ...
    for epoch in range(args.epochs):
        ...
        # --- save best model
        ## select metric
        eval_metric = -valid_results["loss"]
